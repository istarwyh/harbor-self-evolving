...
The card's deterministic Preflight and explicit UI confirmation own execution. A supported read-only `compare` can complete as an audited Operation. Candidate/Evaluator/Gate/handoff actions save draft documents only, not applied resources. `diagnostic-evaluation` and `retry-infrastructure` remain blocked when the workspace has no registered bounded runner with a materialized subset, pinned identities, quotas and cancellation. Do not bypass a blocking card with `harbor_eval_run`, shell commands, or another mutation tool. Never call a blocked preview a successful Job. Production pause/ramp/rollback/deploy are not registered.
