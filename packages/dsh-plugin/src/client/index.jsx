...
import { EvaluationResult, EVALUATION_RESULT_MESSAGES } from './evaluation-result.jsx'
import { deriveArtifactProfile, pipelineStagesForArtifactProfile, sectionsForArtifactProfile } from './product-modes.js'
...
const dictionaries = {
  zh: {
    automaticContextNotReady: 'Harbor 页面尚未加载完成，请稍后重试。', automaticContextFailed: '未能附带 Harbor 页面，消息尚未发送。请重试。',
    savedDraftOnly: '已保存操作草稿，尚未应用到资源；没有启动评测或 Gate。', actionDraft: '操作草稿', checkParameters: '检查参数', confirmActionReview: '我已检查目标、版本、范围和影响；确认仅执行此预览。', confirmAction: '确认此预览', discardDraft: '放弃', draftDiscarded: '草稿已收起，未执行', openDiffEditor: '在编辑器中审阅 Diff', noProductionImpact: '无；不会部署、Gate 或运行评测', draftNotApplied: 'AI 只生成了草稿。选择对应的已保存源文件后，载入编辑区；仍需人工审阅并另行保存。', applyToDraft: '载入待审阅编辑区', selectFiltered: '全选筛选结果（快照）', selectObject: '选择对象', health_all: '全部批次', health_running: '运行中', health_blocked: '全量阻断', health_stalled: '停滞', health_infrastructure: '基础设施异常', health_invalid: '无效分 / 评测异常', health_regressed: 'Candidate 回归', health_gate: 'Gate 待处理', 'health_fresh-baseline': '需要新 Baseline', health_healthy: '未发现阻断', noFilteredJobs: '当前风险筛选没有 Job。', jobSection_summary: '结论', jobSection_trials: '任务与证据', jobSection_pipeline: '执行流程', jobSection_optimization: '改进建议', jobSection_compare: '版本比较', jobSection_evaluator: '评测器', jobSection_artifacts: '原始产物', jobSection_audit: '审计信息', advancedDetails: '实验详情 / 高级信息', askHealth: '这次 Job 是否健康？分数是否有效、可比较？请读取证据，列出最值得先处理的三个问题。', askMetric: '解释这个指标的含义、有效性和覆盖范围，并给出证据。', noMetric: '尚无有效指标；不要把基础设施异常解释成业务 0 分。', attentionCountHint: '按 Job 计数；点击筛选全部结果',
    reviewDiff: '审阅改动', beforeChange: '已保存版本', afterChange: '待保存的新版本', confirmDiff: '我已审阅差异；保存将创建新版本，需要 fresh baseline，不会自动运行评测或 Gate。',
    contextIdentity: '查看完整身份与快照', askHypothesis: '质疑这个假设：证据是否充分，最小验证动作是什么？', askGateReason: '解释这条 Gate 阻断原因及解除条件，不执行 Gate 或发布。', askFinding: '解释这个问题，区分基础设施故障与质量问题，并给出证据。', askAttempt: '分析本次运行过程及失败阶段，不执行重试。', askSource: '审查选中的已保存评测器片段，提出修改建议和 Diff，不保存、不运行。', sourceSelection: '选择源码行后提问', sourceSaved: '引用已保存版本；草稿修改不会进入证据', evaluatorEditExecuted: '当前编辑器绑定到本次 Job 已验证、实际执行的评测器。保存会创建新的评测器与 Stack 版本。', evaluatorForkExecuted: '当前编辑器来自本次 Job 的不可变执行快照。保存会从该快照分叉新版本，不会改写当前或历史版本。', unverifiedAnswer: '尚未取得可验证证据，以下回答不能作为诊断结论。', showUnverified: '查看待核实的 AI 输出', summaryView: '概览', trialsView: 'Trials 与证据', pipelineView: 'Pipeline', optimizationView: '优化假设', artifactsView: '产物', auditView: '审计', attention: '需要关注', healthy: '未发现阻断', healthRisk: '有风险', viewEvidence: '查看证据', pageScope: '当前页统计', selectedCount: '已选择', askSelected: '分析选中对象', clearSelection: '清除选择', allVisible: '选择当前页', health: '健康状态', mainIdentity: '实验身份', compareAction: '对比与 Gate', noEvidenceYet: '证据尚未生成', pipelineHint: 'Pipeline 用于查看集成细节；日常诊断从概览和 Trials 开始。',
    tab: 'Harbor', settings: 'Harbor 自进化',
    heroTitle: '稳住 Agent 的每一次进步',
    heroBody: 'Harbor 固定实验边界；Trial Lifecycle 展示真实运行过程；Score Validity 阻止基础设施故障伪装成业务 0 分。',
    refresh: '刷新', jobs: '评测批次', jobsHint: '点击 Job 后，最多再点一次即可进入对应 Trial 的证据。', formalExperiment: '正式业务实验', formalExperimentHint: '使用固定评测集、你的生成器和你提供的 Evaluator，建立可复现的 Baseline/Candidate 比较。普通实验不会运行 Gate；只有显式选择治理模式才会进入晋级判断。', formalExperimentFlow: '评测集 → 生成器 → 实际执行的评测器 → 指标 → 一个可验证的改进', planFormalExperiment: '规划正式实验', formalExperimentPrompt: '请使用 evolve-agent-with-harbor 帮我规划一个正式业务 Experiment。先确认固定 Dataset、Candidate Generator、用户提供的 Evaluator v2、主指标和 repeat policy，再预览 experiment 模式的 Evaluation Context。不要运行评测、Gate 或修改文件，直到我确认。', workspace: '工作空间', workspaceSelect: '选择 Harbor 工作空间', empty: '还没有评测结果。请评测最近会话，或通过 Harbor Agent 创建评测。',
    ...
    completed: '已完成', partial: '完成但有异常', failed: '读取失败', pending: '等待运行', running: '运行中', 'completed-unscored': '已完成，证据不足无法评分', 'candidate-quality-failed': '候选质量未达标', 'infrastructure-error': '基础设施异常', 'evaluation-error': '评测器异常', 'running-agent': '生成器运行中', evaluating: '评测中', unknown: '状态未知', scored: '已评分', unscored: '未评分',
    ...
    context: 'Context v3', trials: 'Trials', exceptions: '异常', mode: '模式', close: '关闭', retry: '重试', loading: '正在读取…', noData: '暂无数据', currentStatus: '当前状态',
    ...
    evaluatorImplementation: '评测器实现', evaluatorKind: '实现类型', evaluatorProtocol: '接口协议', editableFiles: '允许修改的文件', openFile: '打开', editingFile: '正在修改', editSource: '直接修改当前文件', evaluatorVersion: '新 Evaluator 版本', stackVersion: '新 Stack 版本', saveEvaluator: '保存为新身份', saving: '正在保存…', saved: '已保存；下一步请做元评测并建立新 Baseline。', reloadBeforeSave: '源码已变化，请刷新后再保存。', noEvaluatorInterface: '当前 Stack 还没有 harbor-dsh-evaluator/v2 接口，不能从 UI 安全编辑。', editWarning: '保存只更新源码与身份，不会自动运行评测或 Gate。',
    upgradeStep1: '查看当前 Evaluator、Rubric、Judge、评分合同和代表性误判样本。', upgradeStep2: '创建新的评测器身份、版本和源文件；不覆盖历史评测器。', upgradeStep3: '使用独立、可追溯的 GT 运行元评测，检查 ESF、SCE、RCR、延迟和成本。', upgradeStep4: '更新 Evaluation Stack 身份，并预览 Context v3 变化。', upgradeStep5: '在新分数语义下建立全新 Agent Baseline，再比较后续 Candidate。', evaluatorPrompt: '请使用 evolve-agent-with-harbor 升级当前评测器。先读取 governance 证据，澄清 GT 的来源类型、provenance、维护者和目标元指标，再提出新的不可变评测器身份与 fresh-baseline 方案。在我批准受控改动前，不要修改文件或发起评测。',
    queryTrial: '任务 / Trial', statusLabel: '状态', attempt: '尝试', population: '任务数量',
    experimentIdentity: '本次实验使用了什么', experimentIdentityHint: 'Candidate 自带 ACP 启动入口和锁定依赖；运行时身份随候选内容固定。历史未绑定版本仅供查看，执行前需要迁移为新 Candidate。', immutableCandidateFiles: '候选版本内容', file: '文件', size: '大小', digest: '内容指纹', runtime: '运行时', evaluationStack: 'Evaluation Stack',
  ...
  en: {
    ...
    refresh: 'Refresh', jobs: 'Evaluation jobs', jobsHint: 'Open a Job, then reach Trial evidence in at most one more interaction.', formalExperiment: 'Formal business experiment', formalExperimentHint: 'Use a fixed Dataset, your Generator, and your Evaluator to establish a reproducible Baseline/Candidate comparison. A normal Experiment never runs Gate; promotion is available only in explicit Governed mode.', formalExperimentFlow: 'Dataset → Generator → executed Evaluator → Metrics → one verifiable improvement', planFormalExperiment: 'Plan a formal experiment', formalExperimentPrompt: 'Use evolve-agent-with-harbor to help me plan a formal business Experiment. First confirm a fixed Dataset, Candidate Generator, user-provided Evaluator v2, primary metric, and repeat policy, then preview an experiment-mode Evaluation Context. Do not run an evaluation, Gate, or modify files until I confirm.', workspace: 'Workspace', workspaceSelect: 'Select Harbor workspace', empty: 'No evaluation results yet. Evaluate recent sessions or create an evaluation through the Harbor Agent.',
    ...
    completed: 'Completed', partial: 'Completed with errors', failed: 'Read failed', pending: 'Queued', running: 'Running', 'completed-unscored': 'Completed, insufficient evidence to score', 'candidate-quality-failed': 'Candidate quality failed', 'infrastructure-error': 'Infrastructure error', 'evaluation-error': 'Evaluator error', 'running-agent': 'Generator running', evaluating: 'Evaluating', unknown: 'Unknown status', scored: 'Scored', unscored: 'Unscored',
    ...
    context: 'Context v3', trials: 'Trials', exceptions: 'Exceptions', mode: 'Mode', close: 'Close', retry: 'Retry', loading: 'Loading…', noData: 'No data', currentStatus: 'Current status',
...
function isHistoricalJob(value) {
  ...
  return value?.jobKind === HISTORICAL_JOB_KIND
    ...
    || protocols.has('historical-generation-evaluation-context/v3')
}
...
export function normalizeHarborUiError(value, observedAt = new Date().toISOString()) {
  ...
  const normalizedObservedAt = typeof source.observedAt === 'string' && !Number.isNaN(Date.parse(source.observedAt))
    ...
    : fallbackObservedAt
  const category = /REVISION_CONFLICT|STALE_SELECTION|BINDING_STALE|SOURCE_CONFLICT/i.test(code) || /source changed after it was opened/i.test(message)
    ? 'conflict'
    : /EXPIRED/i.test(code) ? 'expired'
...
function useHarborApi() {
  ...
  return useCallback((route, params = {}, options) => api(route, { ...params, sessionId }, options), [sessionId])
}
...
// Contextual intents are labels plus full prompts, never automatic sends.
...
function ActionDraftCard({ draft, onSourceDraft, onReprepare, onViewComparison, onViewResult, t }) {
  ...
  return <ActionDraftCardView {...{ draft, onSourceDraft, onReprepare, onViewComparison, onViewResult, update, request, t }} ErrorState={HarborErrorState}/>
}
...
export function resolvedUiContext(resolved, sessionId) {
  if (resolved?.schema !== 'harbor-resolved-context/v1') return undefined
  ...
  return context ? { ...context, identities: resolved.context.identities, flags: resolved.context.flags, artifactRevision: resolved.basedOn?.artifactRevision, observedAt: resolved.basedOn?.observedAt } : undefined
}

export function harborDisplayedAnswerBasis(resolved, references, continuation, latest, fallback) {
  const matching = resolved?.contextSnapshotId === latest?.contextSnapshotId ? latest : undefined
  const verified = resolved ? { ...resolved, basedOn: { ...resolved.basedOn, currentRevision: matching?.basedOn?.currentRevision ?? resolved.basedOn?.currentRevision } } : undefined
  return harborAnswerBasis(verified, references, continuation ? undefined : fallback)
}

export function actionDraftContext(draft, sessionId, pageSessionId) {
  const target = draft?.target
  if (!target?.workspace) return undefined
  if (!target.job) return target.kind === 'harbor.workspace/v1' ? buildUiContext({ sessionId, pageSessionId, workspace: target.workspace }) : undefined
  ...
  const selections = (draft.selection ?? []).map(ref => {
    const kind = ref.kind?.replace(/^harbor\.(.+)\/v1$/, '$1')
    return { ...ref, kind, id: ref.id ?? ref.evidenceRef ?? ref.criterion ?? ref.hypothesis ?? ref.trial, stage }
  })
  const context = buildUiContext({ sessionId, pageSessionId, workspace: target.workspace, job: target.job, stage, trial: target.trial, localObject: source, selections: source ? undefined : selections })
...
export function createHarborActionHandlers({ bridge, sessionId, prepareQuestion, onNavigate, t }) {
  ...
  const reprepare = async draft => {
    ...
    if (!prepared && ['conflict', 'expired'].includes(error?.category)) {
      // Do not silently replace an expired subset or old source with a wider
...
function CandidatePanel({ artifacts, t }) {
  ...
  return <>
    <section className="hse-section"><h3>{t('experimentIdentity')}</h3><p className="hse-muted">{t('experimentIdentityHint')}</p><div className="hse-identity-grid">
      ...
      <div className="hse-card"><span>{t('runtime')}</span><b>Harbor {runtime.harbor_version ?? '—'}</b><code>{candidate.runtime?.policy ?? 'unbound'} · Node {candidate.runtime?.node_version ?? '—'} · {context.mode ?? '—'}</code><code>{candidate.runtime?.entrypoint ?? '—'} · {short(candidate.runtime?.lockfile_digest)}</code></div>
    </div></section>
...
function EvaluatorEditor(props) {
  const update = useHarborMutation()
  const sessionId = useContext(HarborSessionContext)
  return <EvaluatorEditorView {...props} sessionId={String(sessionId)} update={update} ErrorState={HarborErrorState} applySourceProposal={applySourceProposal} nextVersion={nextVersion}/>
}
...
function GovernancePanel({ job, workspace, contextFor, setContext, askContext, navigation, proposal, t }) {
  ...
  const receipt = saveReceipt?.requestKey === requestKey ? <SavedEvaluatorNextSteps receipt={saveReceipt.value} historicalJob={job} onPreparePlan={prompt => askContext(contextFor({}), prompt)} t={t}/> : null
  const currentState = state.requestKey === requestKey ? state : { requestKey, status: 'loading' }
  if (currentState.status === 'loading') return <>{receipt}<HarborSkeleton kind="governance" rows={6} label={t('loading')}/></>
  if (currentState.status === 'error') return <>{receipt}<HarborErrorState error={currentState.error} retry={() => void load()} t={t}/></>
  const value = currentState.value
  const evaluator = value.components?.evaluator
  const effectiveEvaluator = value.effectiveEvaluator
  const executedEvaluator = effectiveEvaluator?.executed ?? effectiveEvaluator?.configured
  const effectiveStatus = effectiveEvaluator?.status === 'verified'
    ? t('evaluatorVerified')
    : effectiveEvaluator?.status === 'mismatch'
      ? t('evaluatorMismatch')
      : effectiveEvaluator?.status === 'unattested'
        ? t('evaluatorUnattested')
        : t('evaluatorUnknown')
  ...
  return <>{receipt}{value.savedEvaluatorRecovery?.status === 'UNAVAILABLE' ? <section className="hse-section" role="alert"><p>{t('savedVersionRecoveryUnavailable')}</p><button type="button" className="hse-button" onClick={() => void load()}>{t('refresh')}</button></section> : null}<section className="hse-section"><h3>{t('currentEvaluator')}</h3><p className="hse-muted">{t('governanceHint')}</p><div className="hse-governance-id"><div className="hse-card"><span>{t('effectiveEvaluator')}</span><b>{executedEvaluator?.id ?? '—'} · {executedEvaluator?.version ?? '—'}</b><code>{effectiveStatus}</code></div><div className="hse-card"><span>{t('rubric')}</span><b>{rubric?.id ?? '—'} · {rubric?.version ?? '—'}</b><code>{rubric?.entry ?? '—'}</code></div><div className="hse-card"><span>Judge</span><b>{value.judge?.provider ?? '—'} / {value.judge?.model ?? '—'}</b><code>{judgeIdentityDetails(value.judge)}</code></div></div></section>
    {value.editingPolicy?.identityMatch ? <p className="hse-muted">{t(value.editingPolicy.forkFromExecuted ? 'evaluatorForkExecuted' : 'evaluatorEditExecuted')}</p> : null}<EvaluatorEditor proposal={proposal} job={job} value={value} workspace={workspace} bindingKey={currentState.requestKey} bindingIsCurrent={bindingIsCurrent} reload={load} onSaved={value => setSaveReceipt({ requestKey, value })} t={t}/>{['evaluator', 'rubric'].map(role => <SavedSourceFragment key={role} component={value.components?.[role]} object={value.interactionObjects?.find(ref => ref.sourceRole === role)} contextFor={contextFor} setContext={setContext} askContext={askContext} navigation={navigation} t={t}/>)}
...
function HistoricalMetaEvaluationPanel({ detail, artifacts, t }) {
  ...
  return <section className="hse-section"><h3>{t('meta')}</h3><p className="hse-muted">{t('metaNotRunHint')}</p><div className="hse-grid"><div className="hse-card"><span>{t('currentStatus')}</span><b>{notRun ? t('metaNotRun') : metaEvaluation.status ?? '—'}</b><code>{context.protocol ?? 'historical-generation-evaluation-context/v3'}</code></div><div className="hse-card"><span>Validation report</span><b>{metaEvaluation.validation_report_ref ?? '—'}</b><code>{notRun ? 'Evaluator reliability remains unvalidated' : short(metaEvaluation.digest)}</code></div></div></section>
}
...
function JobIdentityHeader({ context, summary, t }) {
  return <details className="hse-job-identities"><summary>{t('identityDetails')} · {t('progress')}: {summary?.progress?.completed ?? 0}/{summary?.progress?.total ?? summary?.nTrials ?? '—'}</summary><div className="hse-identity-tags">{['candidate', 'dataset', 'context', 'stack'].map(role => {
  ...
  })}</div><div className="hse-identity-flags"><ContextFlags context={context} t={t}/><span>{t('mode')}: {summary?.mode ?? '—'}</span><span>{t('validity')}: {summary?.nValidScores ?? '—'} / {summary?.nTrials ?? '—'}</span></div></details>
}

function JobSummaryPanel({ detail, summary, contextFor, setContext, askContext, navigation, openSection, t }) {
  if (detail?.report) return <EvaluationResult report={detail.report} openSection={openSection} t={t}/>
...
function Workbench({ job, workspace, jobs, close, navigation, navigationRevision, consumeNavigation, restoreView, hasHistory, scrollContainerRef, onViewStateChange, sessionId, pageSessionId, bridge, askContext, t }) {
  ...
  return <aside className="hse-drawer" aria-label={job} onClickCapture={() => consumeNavigation?.(navigation)} onPointerDown={stopRestoredScroll} onWheel={stopRestoredScroll} onKeyDown={stopRestoredScroll}>
    ...
    <JobIdentityHeader context={jobContext} summary={activeJob} t={t}/><div className="hse-workbench"><nav className="hse-object-nav" aria-label={t('mainIdentity')}>{primarySections.map(item => <button type="button" key={item} aria-current={section === item ? 'page' : undefined} onClick={() => openSection(item)}>{t(`jobSection_${item}`)}</button>)}</nav><details className="hse-advanced-nav" open={advancedSections.includes(section) ? true : undefined}><summary>{t('advancedDetails')}</summary><nav className="hse-object-nav">{advancedSections.map(item => <button type="button" key={item} aria-current={section === item ? 'page' : undefined} onClick={() => openSection(item)}>{t(`jobSection_${item}`)}</button>)}</nav></details>{section === 'pipeline' ? <><p className="hse-muted">{t('pipelineHint')}</p><nav className="hse-stage-nav" aria-label={t('stageNav')}>{pipelineStages.map(item => <button type="button" key={item} data-active={stage === item} aria-current={stage === item ? 'step' : undefined} onClick={() => setStage(item)}>{pipelineStages.indexOf(item) + 1}. {historical && item === 'candidate' ? t('historicalTarget') : historical && item === 'dataset' ? t('generationRecords') : t(item)}</button>)}</nav></> : null}{state.status === 'loading' ? <HarborSkeleton kind="workbench" rows={7} label={t('loading')}/> : state.status === 'error' ? <HarborErrorState error={state.error} retry={() => void load()} t={t}/> : <>{state.error ? <HarborErrorState error={state.error} title={state.stale ? t('workbenchStale') : undefined} retry={() => void load()} t={t}/> : null}{!contextSupported ? <div className="hse-capability">{t('capabilityUnavailable')}</div> : null}{content}</>}</div>
  </aside>
}
...
function HistoricalLauncher({ snapshot, reload, onCompleted, t, variant = 'card', pluginVersion }) {
  ...
  const healthKey = (overview?.attention?.blocked ?? 0) > 0
    ? 'health_blocked'
    : ['blocked', 'stalled', 'infrastructure', 'invalid', 'regressed', 'gate', 'fresh-baseline'].some(key => (overview?.attention?.[key] ?? 0) > 0) ? 'healthRisk' : 'healthy'
  const heroStats = [[t('jobs'), overview?.totalJobs ?? '—'], [t('trials'), overview?.totalTrials ?? '—'], [t('exceptions'), overview?.totalExceptions ?? '—']]

  return <>
    {variant === 'hero' ? <section className="hse-hero" style={{ '--ocean-image': `url(${oceanBackground})` }} aria-live="polite">
      <div className="hse-hero-top"><div className="hse-hero-copy"><h1>Harbor{pluginVersion ? <span className="hse-hero-version">v{pluginVersion}</span> : null}</h1><p>{t('heroTitle')}</p></div><div className="hse-hero-actions"><button type="button" className="hse-refresh" onClick={() => void reload()}>{t('refresh')}</button><button type="button" className="hse-hero-primary" disabled={!workspace || state.status === 'previewing'} onClick={() => active ? setOpen(true) : void preview()}>{buttonLabel}</button></div></div>
      <div className="hse-hero-bottom"><div className="hse-hero-health"><span>{t('health')}</span><b>{snapshot ? t(healthKey) : '—'}</b><small>{t('attentionCountHint')}</small></div><div className="hse-stats">{heroStats.map(([label, value]) => <div className="hse-stat" key={label}><span>{label}</span><b>{value}</b></div>)}</div></div>
    </section> : <section className="hse-launch-card" aria-live="polite">
    ...
    {open ? <div className="hse-launch-overlay" role="presentation" onMouseDown={event => event.target === event.currentTarget && close()}><section className="hse-launch-dialog" role="dialog" aria-modal="true" aria-labelledby="hse-historical-title">
      ...
      <div className="hse-launch-body">
        ...
        {state.status === 'ready' && previewValue ? <>
          ...
          <section className="hse-launch-section"><h3>{t('historicalDataPolicy')}</h3><p className="hse-boundary-note">{dataPolicy?.mode === 'source-text-with-secret-redaction' && dataPolicy?.localPaths === 'preserved' ? t('historicalDataPolicyValue') : dataPolicy?.mode ?? '—'}</p><h3>{t('historicalJudgeBoundary')}</h3><p className="hse-boundary-note">{t('historicalJudgeBoundaryDetail')}</p></section>
...
function DashboardSessionView({ t, bridge, sessionId, useInput, inputActions, replaceHarborReference }) {
  ...
  return <HarborSessionContext.Provider value={sessionId}><main ref={rootNode} className="hse-root"><div className="hse-page hse-layout">
    ...
    <div className="hse-main-panel">
      ...
      <HistoricalLauncher variant="hero" snapshot={snapshot} reload={state.load} onCompleted={completedHistorical} t={t} pluginVersion={__HSE_VERSION__}/>
      <section className="hse-experiment-onboarding" aria-labelledby="hse-formal-experiment-title"><div><h3 id="hse-formal-experiment-title">{t('formalExperiment')}</h3><p>{t('formalExperimentHint')}</p><code>{t('formalExperimentFlow')}</code></div><button type="button" disabled={!snapshot?.workspace?.id} onClick={() => void askContext(buildUiContext({ sessionId, pageSessionId, workspace: snapshot.workspace.id }), t('formalExperimentPrompt'))}>{t('planFormalExperiment')}</button></section>
      <section className="hse-attention-filters" aria-label={t('attention')}><div className="hse-health-filters">{ATTENTION_FILTERS.map(filter => <button type="button" key={filter} aria-pressed={attentionFilter === filter} onClick={() => { setAttentionFilter(filter); setOffset(0) }}><span>{t(`health_${filter}`)}</span><b>{snapshot?.overview?.attention?.[filter] ?? '—'}</b></button>)}</div></section>
      ...
      {snapshot?.workspace ? <div className="hse-hook-state"><b>{t('workspace')}: {snapshot.workspace.label}</b></div> : null}
