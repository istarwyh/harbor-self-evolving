...
test('built Web client registers the Evaluation Workbench, Doctor, and sixteen Tool views', async () => {
  ...
  assert.match(source, /mutate\('historical-preview',[\s\S]*limit: 10[\s\S]*mutate\('historical-run'/, 'Historical launch must Preview up to ten Sessions before explicit confirmation')
  assert.match(source, /api\('historical-operation',[\s\S]*onCompleted\(operation\)/, 'background Historical runs must resume, poll, and open the completed Job')
