...
test('built Web client registers the Workbench, invisible input synchronization, Doctor, and nineteen native Tool views', async () => {
  ...
  assert.match(source, /contextSupported = detail\?\.capabilities\?\.contextSupported \?\? detail\?\.capabilities\?\.contextV2/, 'current Candidate/Historical contexts and documented legacy contexts must be recognized through the server capability')
  ...
  assert.match(source, /contextScope = useMemo\([\s\S]*?\[job, workspace, section, stage, navigationRevision\]/, 'Job, workspace, section and accepted navigation changes invalidate the previous child context owner')
  assert.match(source, /if \(childContext\.current === contextScope\) return[\s\S]*?childContext\.current = undefined[\s\S]*?\[bridge, contextScope, sessionId\]/, 'parent effects reset old child context without overwriting a target already published by this view')
  assert.match(source, /function HistoricalLauncher/, 'Workbench must provide a first-class Historical Session launcher')
  assert.match(source, /<HistoricalLauncher variant="hero"/, 'Harbor home must render Historical launch inside the compact Hero')
  assert.match(source, /className="hse-hero"[\s\S]*?<h1>Harbor[\s\S]*?hse-hero-version[\s\S]*?<\/h1>[\s\S]*?hse-hero-health/, 'the compact deep-blue Hero must consolidate Harbor identity and health')
  assert.match(source, /const heroStats = \[\[t\('jobs'\)[\s\S]*t\('trials'\)[\s\S]*t\('exceptions'\)/, 'the compact Hero must show Job, Trial, and exception metrics')
  assert.match(source, /className="hse-attention-filters"/, 'attention filters must remain directly below the consolidated Hero')
  assert.doesNotMatch(source, /function GettingStarted|className="hse-journey"|className="hse-health-summary"/, 'redundant onboarding and standalone health blocks must not return')
  ...
  assert.equal(registrations[1].component.name, 'HarborInputSync', 'the input slot must mount only the invisible reference synchronizer')
  assert.match(source, /function HarborInputSync[\s\S]*?return null\s*\n\}/, 'reference lifecycle synchronization must not render a second Composer panel')
  assert.doesNotMatch(source, /function (?:ContextDock|CopilotDock)|<(?:ContextDock|CopilotDock)\b/, 'the removed context and Copilot panels must not remain in the Workbench')
  assert.match(source, /\.hse-layout\{[^}]*grid-template-columns:minmax\(0,1fr\);/, 'the Workbench must devote its full single-column layout to plugin content')
  assert.doesNotMatch(source, /<AnswerText\b|className="hse-copilot-answer"/, 'assistant responses must remain in the native conversation instead of a duplicate answer surface')
  assert.match(source, /function createHarborActionHandlers/, 'native tool cards must preserve the existing action and recovery handlers')
  assert.match(source, /<OperationTray\b[^>]*\bhideWhenEmpty\b/, 'background operations must remain available in the main plugin page without an empty-state panel')
  assert.match(source, /trustedHarborUiAction\(toolName, value\)/, 'native tool navigation must trust only schema-bound Harbor reader results')
  assert.match(source, /bridge\.navigate\(sessionId, uiAction, \{ force: true \}\)/, 'only an explicit native tool action may force Harbor navigation')
  ...
  assert.match(source, /setContext\(contextFor\(\{\s*trial: selected,\s*\.\.\.\(selected \? \{ detail, \.\.\.focused \} : \{\}\),\s*filters: \{ status, validity \},\s*sort,\s*\}\)\)\s*\}, \[checked, contextFor, detail, focused, selected, setContext, sort, status, validity\]/, 'list and Trial filters or sort must refresh current Context without sending free-text query')
  ...
  assert.match(source, /clearStructuredHarborReferences\(conversation\.input\.for\(actx\)\)/, 'clearing a native Harbor reference must also remove its structured Draft occurrence')
  ...
  assert.doesNotMatch(source, /mutate\('version-update'/, 'Settings must not execute registry packages from the browser')
  assert.match(source, /navigator\.clipboard\.writeText\(value\.command\)/, 'Settings must copy the exact reviewed terminal command')
  assert.match(source, /保留 profile、DSH_HOME、Jobs、运行环境与托管 Python 目录/, 'Settings must disclose the preserved installation identity')
