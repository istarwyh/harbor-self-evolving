...
test('built Web client registers the Workbench, invisible input synchronization, Doctor, and nineteen native Tool views', async () => {
  ...
  assert.match(source, /mutate\('version-update', \{\}\)/, 'Settings must offer one-click Plugin updates')
  assert.match(source, /navigator\.clipboard\.writeText\(value\.command\)/, 'Settings must retain the exact terminal command as a fallback')
  assert.match(source, /updateInstalled/, 'Settings must require a restart after installing an update')
