...
test('built Web client registers the AI-native Workbench, Context Dock, Doctor, and nineteen Tool views', async () => {
  ...
  assert.match(source, /const basis = harborAnswerBasis\(resolved \?[\s\S]*currentRevision:[\s\S]*references, ui\.lastSent\?\.context\)/, 'Copilot must render answer basis from trusted typed results and the frozen submitted context')
