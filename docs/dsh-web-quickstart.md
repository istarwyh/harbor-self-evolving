...
版本组合：DSH `latest`、Plugin + Skill `0.9.3`、Harbor Adapter `0.9.3`、Harbor `0.21.x`。
...
npx --yes dsh-harbor-evolution@0.9.3 setup --project-root "$PWD"
```
...
| 工具存在但 Skill 不出现 | 确认 `harbor-evolution` 版本为 `0.9.3` 并重启 |
| `spawn harbor ENOENT` | 重新运行 setup 保存绝对 CLI 路径 |
| Harbor 找不到 `dsh-evolution` 或 `dsh-historical-evaluation` | Adapter 与 Harbor 不在同一 venv，或仍是旧版；重新运行 0.9.3 setup |
