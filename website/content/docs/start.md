...
- Harbor `>=0.21,<0.22`.
- Docker only if you explicitly choose container execution; 0.9.6 defaults to Host.
...
For a version-pinned installation, replace `latest` with `0.9.6`. Setup writes the selected DSH profile dependency, configures the Harbor project integration, installs the compatible Python Adapter and exposes the bundled `evolve-agent-with-harbor` Skill. Follow the exact restart command printed by setup.
...
1. the selected profile depends on exact registry version `"dsh-harbor-evolution": "0.9.6"`, not `link:...`;
...
Source builds can contain unreleased behavior and must not be presented as the formal 0.9.6 package. The current untagged one-click update work is one such Development preview.
