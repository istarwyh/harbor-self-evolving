...
- Harbor `>=0.21,<0.22`。
- 只有显式选择容器执行时才需要 Docker；0.9.6 默认 Host。
...
需要固定版本时，把 `latest` 换成 `0.9.6`。Setup 会写入选定 DSH profile 依赖、配置 Harbor 项目集成、安装兼容 Python Adapter，并暴露内置 `evolve-agent-with-harbor` Skill。随后执行 setup 打印的精确重启命令。
...
1. 选定 profile 依赖精确 registry 版本 `"dsh-harbor-evolution": "0.9.6"`，而不是 `link:...`；
...
源码 build 可能包含未发布行为，不能当作正式 0.9.6 包。本地未打 tag 的一键更新工作就是开发预览之一。
